...
class LoginState(rx.State):
    ...
    def login(self):
        ...
        result = DB_SERVICE.login(self.email, self.password)
...
class CreateAccountState(rx.State):
    ...
    def create_account(self):
        ...
        result = DB_SERVICE.signup(self.email, self.password, self.birthdate, self.location)
        if "success" in result:
            return rx.redirect("/")
        else:
...
class TiendaState(rx.State):
    ...
    def load_products(self):
        ...
        try:
            ...
            mobo_amd_data = DB_SERVICE.get_products_by_category_and_brand("mobo", "amd")
            self.mobos_amd = [self._to_producto(p) for p in mobo_amd_data]
            
            # Cargar Motherboards Intel
            mobo_intel_data = DB_SERVICE.get_products_by_category_and_brand("mobo", "intel")
            self.mobos_intel = [self._to_producto(p) for p in mobo_intel_data]
            
            # Cargar CPUs AMD
            cpu_amd_data = DB_SERVICE.get_products_by_category_and_brand("cpu", "amd")
            self.cpus_amd = [self._to_producto(p) for p in cpu_amd_data]
            
            # Cargar CPUs Intel
            cpu_intel_data = DB_SERVICE.get_products_by_category_and_brand("cpu", "intel")
            self.cpus_intel = [self._to_producto(p) for p in cpu_intel_data]
